...
<div>
    ...
    <div class="ui text container" style="max-width:50rem">
        ...
        <template v-if="sentenceIndex < transcription.sentences.length">
            <textarea style="margin-top:1rem" class="textarea is-primary" v-model="input"></textarea>
            <!-- Options card -->
...
export default {
  ...
  computed: {
    ...
    currentSentence () {
        if (this.sentenceIndex >= this.transcription.sentences.length)
            return ['', '']

        return this.transcription.sentences[this.sentenceIndex]
    },
    progress () { return this.sentenceIndex / this.words.length },
    sentences () {
  ...
  watch: {
    input () {
        ...
        if (this.input.trim() === this.currentSentence[0])  {
            this.sentenceIndex++
            this.input = ''
            this.overallErrors += Math.max(this.wordErrors,0)
